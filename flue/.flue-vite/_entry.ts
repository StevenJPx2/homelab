...
import * as handler_homelab_0 from "/Users/stevenjohn/Documents/Projects/homelab/flue/src/agents/homelab.ts";
import * as workflow_ask_0 from "/Users/stevenjohn/Documents/Projects/homelab/flue/src/workflows/ask.ts";
...
const agentIdentities = {
  "homelab": { bindingName: "FLUE_HOMELAB_AGENT", className: "FlueHomelabAgent" },
};
const workflowIdentities = {
  "ask": { bindingName: "FLUE_ASK_WORKFLOW", className: "FlueAskWorkflow" },
};

const userCloudflare = {};
const reservedCloudflareExportNames = new Set(["FlueHomelabAgent","FlueAskWorkflow","FlueRegistry"]);
...
const agentExtension0 = resolveCloudflareExtension(agentModules["homelab"], "homelab", 'Agent');
const FlueHomelabAgent = class FlueHomelabAgent extends agentExtension0.base(Agent) {
  constructor(ctx, env) {
    const prepared = cloudflareAgents.prepare({ storage: ctx.storage, className: "FlueHomelabAgent", agentName: "homelab" });
    super(ctx, env);
...
const WrappedFlueHomelabAgent = agentExtension0.wrap(FlueHomelabAgent);
export { WrappedFlueHomelabAgent as FlueHomelabAgent };
const workflowExtension0 = resolveCloudflareExtension(workflowModules["ask"], "ask", 'Workflow');
const FlueAskWorkflow = class FlueAskWorkflow extends workflowExtension0.base(Agent) {
  async onRequest(request) {
    return dispatchWorkflow(request, this, "ask");
  }

  async onFiberRecovered(ctx) {
    if (ctx.name?.startsWith('flue:workflow:')) {
      return handleFlueWorkflowFiberRecovered(ctx, this, "ask");
    }
...
const WrappedFlueAskWorkflow = workflowExtension0.wrap(FlueAskWorkflow);
export { WrappedFlueAskWorkflow as FlueAskWorkflow };
